...
#         print(f"=== {self.country.upper()} - DATA SUMMARY ===")
...
#             print(f"{col}: {outlier_flags[col].sum()}")
...
#                 self.clean_df = self.clean_df[~outlier_flags[f'{col}_outlier']]
        
#         print(f"Original data: {len(self.df)} rows")
#         print(f"Cleaned data: {len(self.clean_df)} rows")
...
#         fig.suptitle(f'{self.country} - Time Series Analysis', fontsize=16)
        
#         # GHI over time
#         axes[0,0].plot(self.clean_df['Timestamp'], self.clean_df['GHI'], alpha=0.7)
#         axes[0,0].set_title('GHI Over Time')
#         axes[0,0].set_ylabel('GHI (W/m²)')
#         axes[0,0].tick_params(axis='x', rotation=45)
        
#         # DNI over time
#         axes[0,1].plot(self.clean_df['Timestamp'], self.clean_df['DNI'], alpha=0.7, color='orange')
#         axes[0,1].set_title('DNI Over Time')
#         axes[0,1].set_ylabel('DNI (W/m²)')
#         axes[0,1].tick_params(axis='x', rotation=45)
        
#         # DHI over time
#         axes[1,0].plot(self.clean_df['Timestamp'], self.clean_df['DHI'], alpha=0.7, color='green')
#         axes[1,0].set_title('DHI Over Time')
#         axes[1,0].set_ylabel('DHI (W/m²)')
#         axes[1,0].tick_params(axis='x', rotation=45)
        
#         # Temperature over time
#         axes[1,1].plot(self.clean_df['Timestamp'], self.clean_df['Tamb'], alpha=0.7, color='red')
...
#         """Analyze impact of cleaning on module readings"""
#         if 'Cleaning' in self.df.columns:
#             cleaning_effect = self.df.groupby('Cleaning')[['ModA', 'ModB']].mean()
            
#             fig, axes = plt.subplots(1, 2, figsize=(12, 5))
#             cleaning_effect[['ModA', 'ModB']].plot(kind='bar', ax=axes[0])
#             axes[0].set_title('Average Module Readings by Cleaning Status')
#             axes[0].set_ylabel('Module Reading')
...
#                 axes[1].set_ylabel('Module Reading')
#             plt.tight_layout()
#             plt.show()
...
#         available_cols = [col for col in corr_cols if col in self.clean_df.columns]
        
#         plt.figure(figsize=(10, 8))
#         correlation_matrix = self.clean_df[available_cols].corr()
#         sns.heatmap(correlation_matrix, annot=True, cmap='coolwarm', center=0,
#                    square=True, fmt='.2f')
#         plt.title(f'{self.country} - Variable Correlations')
#         plt.tight_layout()
#         plt.show()
...
#         # WS vs GHI
#         if 'WS' in self.clean_df.columns and 'GHI' in self.clean_df.columns:
#             axes[0,0].scatter(self.clean_df['WS'], self.clean_df['GHI'], alpha=0.5)
#             axes[0,0].set_xlabel('Wind Speed (m/s)')
#             axes[0,0].set_ylabel('GHI (W/m²)')
#             axes[0,0].set_title('Wind Speed vs GHI')
        
#         # RH vs Tamb
#         if 'RH' in self.clean_df.columns and 'Tamb' in self.clean_df.columns:
#             axes[0,1].scatter(self.clean_df['RH'], self.clean_df['Tamb'], alpha=0.5, color='green')
...
#         # RH vs GHI
#         if 'RH' in self.clean_df.columns and 'GHI' in self.clean_df.columns:
#             axes[1,0].scatter(self.clean_df['RH'], self.clean_df['GHI'], alpha=0.5, color='red')
#             axes[1,0].set_xlabel('Relative Humidity (%)')
#             axes[1,0].set_ylabel('GHI (W/m²)')
#             axes[1,0].set_title('RH vs GHI')
        
#         # WSgust vs GHI
#         if 'WSgust' in self.clean_df.columns and 'GHI' in self.clean_df.columns:
#             axes[1,1].scatter(self.clean_df['WSgust'], self.clean_df['GHI'], alpha=0.5, color='purple')
...
#             axes[0,0].hist(self.clean_df['GHI'], bins=50, alpha=0.7, edgecolor='black')
...
#             axes[0,1].hist(self.clean_df['WS'], bins=50, alpha=0.7, edgecolor='black', color='orange')
#             axes[0,1].set_xlabel('Wind Speed (m/s)')
#             axes[0,1].set_ylabel('Frequency')
#             axes[0,1].set_title('Wind Speed Distribution')
        
#         # Temperature distribution
#         if 'Tamb' in self.clean_df.columns:
#             axes[1,0].hist(self.clean_df['Tamb'], bins=50, alpha=0.7, edgecolor='black', color='red')
#             axes[1,0].set_xlabel('Temperature (°C)')
#             axes[1,0].set_ylabel('Frequency')
#             axes[1,0].set_title('Temperature Distribution')
        
#         # RH distribution
#         if 'RH' in self.clean_df.columns:
#             axes[1,1].hist(self.clean_df['RH'], bins=50, alpha=0.7, edgecolor='black', color='green')
#             axes[1,1].set_xlabel('Relative Humidity (%)')
#             axes[1,1].set_ylabel('Frequency')
#             axes[1,1].set_title('RH Distribution')
...
#         if all(col in self.clean_df.columns for col in ['GHI', 'Tamb', 'RH']):
#             plt.figure(figsize=(10, 6))
            
#             # Normalize RH for bubble sizes
#             rh_normalized = (self.clean_df['RH'] - self.clean_df['RH'].min()) / \
#                            (self.clean_df['RH'].max() - self.clean_df['RH'].min())
#             bubble_sizes = 50 + rh_normalized * 200
            
#             scatter = plt.scatter(self.clean_df['Tamb'], self.clean_df['GHI'], 
#                                 s=bubble_sizes, alpha=0.6, c=self.clean_df['RH'], 
#                                 cmap='viridis')
#             plt.colorbar(scatter, label='Relative Humidity (%)')
#             plt.xlabel('Ambient Temperature (°C)')
#             plt.ylabel('GHI (W/m²)')
#             plt.title(f'{self.country} - GHI vs Temperature (Bubble size = RH)')
#             plt.tight_layout()
#             plt.show()
    
#     def comprehensive_analysis(self):
#         """Run complete EDA pipeline"""
#         print(f"🚀 Starting Comprehensive EDA for {self.country}")
#         print("="*50)
...
#         print(f"✅ EDA completed for {self.country}")
